            ...
            <div class="flex flex-col gap-2">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex flex-col gap-0.5 pt-1 flex-shrink-0">
                        <button @click="$emit('move-up')" :disabled="isFirst" :class="[
                        ...
                        <button @click="$emit('move-down')" :disabled="isLast" :class="[
                            'p-0.5 rounded transition',
                            isLast
                                ? 'text-slate-300 cursor-not-allowed'
                                : 'text-slate-400 hover:text-teal-600 hover:bg-teal-50'
                    ...
                    <div class="flex flex-col items-center w-20 shrink-0 gap-2">
                        <div
                            ...
                            <input :value="item.time"
                                @input="updateItem('time', ($event.target as HTMLInputElement).value)" type="time"
                    ...
                    <div class="flex flex-col gap-2">
                        <a v-if="item.location && item.location.trim()" :href="googleMapLink" target="_blank"
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="relative flex items-center gap-2">
                                <!-- 地區輸入前的座標圖示：即使天氣載入中也維持為座標圖示，不切換為 loading -->
                                ...
                                <input :value="item.region"
                                    ...
                                    :disabled="isWeatherLoading || isDayWeatherLoading" @blur="$emit('region-change')"
                                    @keyup.enter="$emit('region-change')" :class="[
                                ...
                                <button v-if="item.region && !isWeatherLoading && !isDayWeatherLoading"
                                    @mousedown.prevent @click.stop="$emit('clear-region')"
                        ...
                        <div class="flex-shrink-0 w-[100px]">
                            <!-- 載入中狀態 -->
                            <div v-if="isWeatherLoading || (isDayWeatherLoading && item.region?.trim())"
                ...
                <div v-if="item.type === 'food'" class="p-2 bg-orange-50 rounded-lg border border-orange-100/50 mt-1">
                    ...
                    >
                        <template v-for="rec in recommendations" :key="rec.name">
                        <button
                                v-if="!isErrorRecommendation(rec)"
...
// 美食推薦相關函數 - 已移除前端功能，保留程式碼
// type Rec = { name: string; location: string };
// const isErrorRecommendation = (rec: Rec): boolean => {
//     return (
//         rec.name.startsWith('找不到地點') ||
//         rec.name.startsWith('找不到推薦') ||
//         rec.name.startsWith('搜尋失敗')
//     );
// };
